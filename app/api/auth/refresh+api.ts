  ...
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      try {
        ...
        platform = ((formData as any).get("platform") as string) || "native";

        // For native clients, get refresh token from form data
        if (platform === "native" && (formData as any).get("refreshToken")) {
          refreshToken = (formData as any).get("refreshToken") as string;
        }
      } catch (e) {
